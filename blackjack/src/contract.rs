...
use abi::blackjack::{blackjack_channel, PlayChainStatus, MAX_BLACKJACK_PLAYERS};
use abi::deck::Deck;
...
impl Contract for BlackjackContract {
    ...
    async fn instantiate(&mut self, argument: Self::InstantiationArgument) {
        ...
        // TODO: check issue with argument.
        // TODO: when not being used, it can caused the app didn't get added to the chain on deployment.
    ...
    async fn execute_operation(&mut self, operation: Self::Operation) -> Self::Response {
        match operation {
            // * User Chain
            BlackjackOperation::Subscribe { chain_id } => {
                self.message_manager(chain_id, BlackjackMessage::Subscribe);
            }
            BlackjackOperation::Unsubscribe { chain_id } => {
